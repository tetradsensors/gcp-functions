...
def interpolateQueryLocations(lat_lo, lat_hi, lon_lo, lon_hi, lat_size, lon_size, lat_res=None, lon_res=None): 
#    lat_step = (lat_hi-lat_low)/float(lat_size)
#    lon_step = (lon_hi-lon_low)/float(lon_size)
    # lon_range = np.arange(lon_lo, lon_hi, lon_res)
    # lat_range = np.arange(lat_lo, lat_hi, lat_res)
    lat_range = np.linspace(lat_lo, lat_hi, num=lat_size, endpoint=True)
    lon_range = np.linspace(lon_lo, lon_hi, num=lon_size, endpoint=True)
